...
<div class="container py-5">
    ...
    @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show rounded-3 shadow-sm">{{ session('success') }}<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
    @endif
    @if(session('info'))
        <div class="alert alert-info alert-dismissible fade show rounded-3 shadow-sm">{{ session('info') }}<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
    @endif
    @if(session('error'))
        <div class="alert alert-danger alert-dismissible fade show rounded-3 shadow-sm">{{ session('error') }}<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
    @endif
    ...
    @else
        <div class="alert alert-warning rounded-3 shadow-sm">
            You don't have an active membership. <a href="{{ route('membership.index') }}" class="alert-link fw-semibold">Subscribe now</a> to continue.
        </div>
    @endif

    <h5 class="mb-3 fw-bold">Payment History</h5>
