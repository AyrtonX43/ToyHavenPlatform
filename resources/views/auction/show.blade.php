...
<div class="container py-4" x-data="auctionLive()" x-init="init()">
    ...
    {{-- Toast notification --}}
    <div x-show="toast.show" x-transition.opacity.duration.300ms class="toast-notification" x-cloak style="display:none;">
    ...
    <div x-show="snipeAlert" x-transition class="alert alert-info alert-dismissible snipe-alert mb-3" x-cloak style="display:none;">
        ...
        <div class="col-lg-4">
            <div class="auction-bid-panel sticky-top">
                ...
                <div x-show="!isEnded">
                    ...
                    @auth
                        @if($auction->user_id !== auth()->id())
                            <div x-show="bidMessage" x-transition class="mb-2" x-cloak style="display:none;">
                ...
                {{-- Ended Auction State --}}
                <div x-show="isEnded" x-cloak style="display:none;">
                    <p class="mb-2"><strong>Final price:</strong> <span class="amount d-block" x-text="currentBidFormatted"></span></p>

                    <div x-show="auctionOutcome === 'reserve_not_met'" x-cloak style="display:none;" class="mb-2">
                        <p class="text-warning mb-0"><i class="bi bi-exclamation-triangle me-1"></i>Reserve not met. No winner.</p>
                    </div>
                    <div x-show="auctionOutcome === 'no_bids'" x-cloak style="display:none;" class="mb-2">
                        <p class="text-muted mb-0">No bids were placed.</p>
                    </div>

                    {{-- Server-rendered ended content (only rendered by Blade when auction is actually ended) --}}
                    @if($auction->status === 'ended')
                        @auth
                            @php
                                $bladePaymentLink = null;
                                $bladePaymentIsPending = false;
                                $bladeIsWinner = $auction->winner_id === auth()->id();
                            ...
                            @if($bladeIsWinner && $auction->meetsReserve())
                                <div class="mt-3 p-4 rounded-3 text-center winner-reveal" style="background:#ccfbf1;border:1px solid #0d9488;">
                                    ...
                                    @else
                                        ...
                                        <button class="btn btn-outline-primary w-100 rounded-pill mt-1" onclick="window.location.reload()">
                            ...
                            @if($auction->user_id !== auth()->id() && $auction->winner_id && $auction->winner_id !== auth()->id() && $auction->auction_outcome === 'sold')
                                <div class="mt-3 p-3 rounded-3 text-center bg-light">
                                    ...
                                    <p class="mb-0 text-muted small">at ₱{{ number_format($auction->winning_amount, 2) }}</p>
                                </div>
                            @endif

                            @if($auction->user_id === auth()->id() && $auction->auction_outcome === 'sold')
                                <div class="mt-3 p-3 rounded-3" style="background:#eff6ff;border:1px solid #3b82f6;">
                                    <p class="mb-1 fw-semibold text-primary"><i class="bi bi-cash-coin me-1"></i>Your Auction Sold!</p>
                                    <p class="mb-1 small">Winner bid: <strong>₱{{ number_format($auction->winning_amount, 2) }}</strong></p>
                                    @if($auction->payment)
