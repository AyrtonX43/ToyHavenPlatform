...
@section('title', 'Add New Product - ToyHaven')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <form action="{{ route('seller.products.store') }}" method="POST" enctype="multipart/form-data" id="productForm" onsubmit="return validateAndSubmitForm(event)">
        @csrf
            ...
            <div class="col-md-8">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" id="product_name" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" required>
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="product_description" name="description" class="form-control @error('description') is-invalid @enderror" rows="5" required>{{ old('description') }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" id="product_brand" name="brand" class="form-control @error('brand') is-invalid @enderror" value="{{ old('brand') }}">
                                @error('brand')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" id="product_sku" name="sku" class="form-control @error('sku') is-invalid @enderror" value="{{ old('sku') }}" required>
                                @error('sku')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-lg-5 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" id="product_price" name="price" step="0.01" min="0" max="9999999.99" class="form-control @error('price') is-invalid @enderror" value="{{ old('price') }}" placeholder="0.00" required onblur="formatPriceInput(this)">
                        ...
                        <input type="hidden" id="platform_fee_percentage" name="platform_fee_percentage" value="{{ old('platform_fee_percentage', 5.00) }}">
                        ...
                        <div id="priceBreakdown" class="card border" style="display: none;">
                            ...
                            <input type="hidden" id="base_price_hidden" name="base_price" value="">
                        </div>
                        ...
                        <div class="mt-4 pt-3 border-top">
                            ...
                            <div id="amazonReferenceDisplay" class="mt-3" style="display: none;">
                                <div class="alert alert-info">
                                    ...
                                    <input type="hidden" name="amazon_reference_price" id="amazon_reference_price_hidden">
                                    <input type="hidden" name="amazon_reference_image" id="amazon_reference_image_hidden">
                                    <input type="hidden" name="amazon_reference_url" id="amazon_reference_url_hidden">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card mb-4 shadow-sm">
                    <div class="card-header bg-light d-flex justify-content-between align-items-center py-3">
                        <div>
                            ...
                            <p class="text-muted small mb-0">Add variants like Color, Size, or Style. Prices are based on your product base price and can be adjusted per option; customer price includes platform fee and VAT.</p>
                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <p class="small text-muted mt-2 mb-0">
                            <i class="bi bi-info-circle me-1"></i> Customer price = (base price + adjustment) + platform fee + 12% VAT. Add variants manually using the button below.
                        </p>
                ...
                <input type="hidden" name="variations_json" id="variations_json" value="{{ old('variations_json', '[]') }}">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="uploadedImagesPreview" class="mt-3" style="display: none;">
                                <label class="form-label">Preview (Your Uploads)</label>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="url" id="product_video_url" name="video_url" class="form-control @error('video_url') is-invalid @enderror" value="{{ old('video_url') }}" placeholder="https://www.youtube.com/watch?v=... or video URL">
                            <small class="text-muted">Enter a video URL (YouTube, Vimeo, etc.)</small>
                            @error('video_url')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
function renderVariationsTable() {
    ...
    tbody.querySelectorAll('.variation-type').forEach(function(el, i) {
        el.onchange = function() { variationsData[i].variation_type = this.value; syncVariationsToInput(); };
    });
    tbody.querySelectorAll('.variation-value').forEach(function(el, i) {
        el.oninput = function() { variationsData[i].variation_value = this.value; syncVariationsToInput(); };
    });
    ...
    tbody.querySelectorAll('.variation-stock').forEach(function(el, i) {
        el.oninput = function() { variationsData[i].stock_quantity = this.value; syncVariationsToInput(); };
    });
    syncVariationsToInput();
}
    ...
    document.getElementById('searchByUrlBtn').addEventListener('click', function() {
        ...
        .then(data => {
            if (data.success && data.product) {
                ...
                // Prepare product data with price_php and variations (so "Use as Reference" has variants without a second fetch)
                const productData = {
                    ...product,
        ...
        .then(data => {
            if (data.success && data.results && data.results.length > 0) {
                ...
                data.results.forEach((product, index) => {
                    ...
                    const productImage = imageUrl ? `<img src="${imageUrl}" alt="${(product.title || 'Product').replace(/"/g, '&quot;')}" class="img-thumbnail" style="max-width: 180px; max-height: 180px; width: 180px; height: 180px; object-fit: contain; cursor: pointer; background: #f8f9fa;" onclick="openImageViewer('${imageUrl}')" title="Click to view HD/HDR quality" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\\'bg-light d-flex align-items-center justify-content-center\\' style=\\'width: 180px; height: 180px;\\'><i class=\\'bi bi-image text-muted\\'></i></div>';">` : '<div class="bg-light d-flex align-items-center justify-content-center" style="width: 180px; height: 180px;"><i class="bi bi-image text-muted"></i></div>';
                    ...
                    const productData = {
                        ...
                        images: product.images && Array.isArray(product.images) ? product.images : (product.image || imageUrl ? [product.image || imageUrl] : [])
                    };
                        ...
                        <div class="list-group-item" id="product-result-${productAsin}" data-asin="${productAsin}">
                            <div class="d-flex gap-3">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    ${product.brand ? `<p class="mb-1 text-muted small">Brand: <strong>${product.brand}</strong></p>` : ''}
                                    <button type="button" id="btn-use-ref-${productAsin}" class="btn btn-sm btn-primary" onclick="setAmazonReferenceFromData('${product.asin || ''}', '${(product.title || '').replace(/'/g, "\\'")}', '${pricePhp}', '${product.url || ''}', '${imageUrl || product.image || ''}', '${(product.brand || '').replace(/'/g, "\\'")}')">
                                        <i class="bi bi-check me-1"></i> Use as Reference
                                    </button>
                                </div>
                            </div>
...
function fetchFullProductDetails(asin, productData) {
    ...
    // If we already have full product from URL search (with variations), use it and skip second fetch
...
function setAmazonReference(product) {
    ...
    const categoryCheckboxes = document.querySelectorAll('.category-selection-checkbox');
    categoryCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            updateCategorySelectionCount();
            // Remove required attribute from all checkboxes if at least one is selected
            if (document.querySelectorAll('.category-selection-checkbox:checked').length > 0) {
                categoryCheckboxes.forEach(cb => {
                    cb.removeAttribute('required');
                });
            } else {
                // Add required to first checkbox if none selected
                categoryCheckboxes[0]?.setAttribute('required', 'required');
            }
        });
    });
    ...
    if (priceFloat > 0) {
        ...
        // Store base price (original price from Amazon before fees/tax)
        document.getElementById('base_price_hidden').value = priceFloat.toFixed(2);
        
        // Set the base price in the input field (user can adjust it)
        // The input field shows base price, and breakdown shows the calculation
        document.getElementById('product_price').value = priceFloat.toFixed(2);
        ...
        // Show a message that user can adjust the price
        const priceInput = document.getElementById('product_price');
        const helpText = priceInput.parentElement.parentElement.querySelector('small.text-muted');
        if (helpText) {
            helpText.innerHTML = '<span class="text-success"><i class="bi bi-info-circle me-1"></i>Price calculated from Amazon reference. You can adjust it using the buttons or by typing directly.</span>';
        }
    }
...
function calculatePriceBreakdown(basePrice, skipInputUpdate) {
    ...
    if (!skipInputUpdate && !isPriceFromAmazon) {
...
function clearAmazonReference() {
    ...
    // Update images required indicator
...
function validateAndSubmitForm(event) {
    ...
    const importedImageUrls = document.getElementById('imported_image_urls').value;
    const importedVideoUrls = document.getElementById('imported_video_urls').value;
    const basePrice = document.getElementById('base_price_hidden').value;
    const finalPrice = document.getElementById('final_price_hidden').value;
    
    console.log('Form Submission Data:', {
        importedImageUrls: importedImageUrls,
        importedVideoUrls: importedVideoUrls,
        basePrice: basePrice,
        finalPrice: finalPrice,
        productPrice: document.getElementById('product_price').value,
        amazonReferencePrice: document.getElementById('amazon_reference_price_hidden').value
    });
    
    // Ensure base_price is set if not already set
    if (!basePrice && document.getElementById('product_price').value) {
        const enteredPrice = parseFloat(document.getElementById('product_price').value) || 0;
        if (enteredPrice > 0) {
            document.getElementById('base_price_hidden').value = enteredPrice.toFixed(2);
        }
    }
