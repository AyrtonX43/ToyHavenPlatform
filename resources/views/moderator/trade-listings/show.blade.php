...
<style>
.listing-view-image-wrap { background: #f8f9fa; border-radius: 8px; padding: 1.5rem; display: flex; align-items: center; justify-content: center; min-height: 320px; }
.listing-view-image-wrap img { max-width: 100%; max-height: 600px; width: auto; height: auto; object-fit: contain; }
.listing-thumb { width: 64px; height: 64px; object-fit: contain; border: 2px solid #dee2e6; border-radius: 6px; cursor: pointer; padding: 4px; background: #fff; }
.listing-thumb:hover, .listing-thumb.active { border-color: #0d6efd; box-shadow: 0 0 0 2px rgba(13, 110, 253, 0.25); }
...
<nav aria-label="breadcrumb" class="mb-4">
    <ol class="breadcrumb mb-0">
        ...
        <li class="breadcrumb-item"><a href="{{ route('moderator.trade-listings.index') }}">Trade Listings</a></li>
        ...
        <div class="card shadow-sm border-0">
            <div class="card-body p-0">
                @php
                    ...
                    $mainSrc = $useListingImages ? asset('storage/' . $listingImages->first()->image_path) : ($fallbackSrc ?? ($productImages->count() > 0 ? asset('storage/' . $productImages->first()->image_path) : null));
                @endphp
                @if($mainSrc)
                ...
                <div class="d-flex gap-2 p-3 overflow-auto flex-wrap border-top" id="thumbRow">
                    ...
                    @php $imgPath = $img->image_path ?? null; $imgSrc = $imgPath ? asset('storage/' . $imgPath) : null; @endphp
                    @if($imgSrc)
                ...
                @endif
                <div class="px-3 pb-3">
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="btnViewFullImage"><i class="bi bi-arrows-fullscreen me-1"></i> View Full Image</button>
                </div>
                @else
                <div class="listing-view-image-wrap">
                    <div class="text-center text-muted py-5"><i class="bi bi-image display-4 d-block mb-2"></i><span>No image available</span></div>
                </div>
                @endif
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="row g-3 mb-4">
                    <div class="col-sm-6">
                        ...
                        <div class="info-value">{{ str_replace('_', ' ', ucfirst($listing->trade_type ?? 'barter')) }}</div>
                    </div>
                    @if($listing->category)
                    <div class="col-sm-6">
                ...
                <div class="d-flex gap-2 pt-2">
                    <form action="{{ route('moderator.trade-listings.approve', $listing->id) }}" method="POST" class="d-inline">
                        @csrf
                        <button type="submit" class="btn btn-success"><i class="bi bi-check-circle me-1"></i> Approve Listing</button>
                    </form>
                    <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#rejectModal" data-reject-url="{{ route('moderator.trade-listings.reject', $listing->id) }}" data-listing-title="{{ e($listing->title) }}">
...
<div class="card shadow-sm border-0 mt-4">
    ...
    <div class="card-body p-0">
        <div id="moderator-listing-map" style="height: 280px; width: 100%;"></div>
        <div class="p-3 border-top"><small class="text-muted"><i class="bi bi-geo-alt me-1"></i>{{ $listing->location }}</small></div>
    </div>
</div>
@endif
        ...
        <div class="list-group list-group-flush">
            ...
            <div class="list-group-item px-0">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        @php $offererName = $offer->offerer ? $offer->offerer->name : ($offer->offererSeller ? $offer->offererSeller->business_name : 'Unknown'); @endphp
...
(function() {
    ...
    function setMainImage(src, el) {
        if (mainImg && src) { mainImg.src = src; mainImg.setAttribute('data-full-src', src); }
        if (thumbRow) thumbRow.querySelectorAll('.listing-thumb').forEach(function(t) { t.classList.remove('active'); });
    ...
    if (thumbRow) {
        thumbRow.addEventListener('click', function(e) {
            ...
            if (t && t.dataset.src) setMainImage(t.dataset.src, t);
        });
    }
    if (mainImg) mainImg.addEventListener('click', function() { openImageModal(mainImg.getAttribute('data-full-src') || mainImg.src); });
    if (btnViewFull) btnViewFull.addEventListener('click', function() { openImageModal(mainImg ? (mainImg.getAttribute('data-full-src') || mainImg.src) : null); });
    ...
    var mapEl = document.getElementById('moderator-listing-map');
    var locationText = @json($listing->location ?? '');
    if (mapEl && locationText) {
        var defaultCenter = [14.5995, 120.9842];
        var map = L.map('moderator-listing-map').setView(defaultCenter, 12);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '&copy; OpenStreetMap' }).addTo(map);
        fetch('https://nominatim.openstreetmap.org/search?format=json&q=' + encodeURIComponent(locationText) + '&limit=1', { headers: { 'Accept': 'application/json', 'User-Agent': 'ToyHavenPlatform/1.0' } })
        ...
        .then(function(data) {
            ...
            } else { L.marker(defaultCenter).addTo(map); }
        }).catch(function() { L.marker(defaultCenter).addTo(map); });
    }
})();
</script>
@endpush
